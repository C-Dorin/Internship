...
function MorphingDialogContent({ children, className, style }: MorphingDialogContentProps) {
	...
	return (
		<motion.div
			...
			layoutId={`dialog-${uniqueId}`}
			className={cn(
				'overflow-hidden rounded-md border bg-background p-2 text-foreground shadow-md border-muted dark:border-muted dark:bg-background dark:text-foreground',
				className
			)}
			style={style}
			role="dialog"
			...
			{isOpen && (
				<>
					<motion.div
						...
						className="fixed inset-0 h-full w-full bg-background backdrop-blur-xs dark:bg-background"
						initial={{ opacity: 0 }}
						animate={{ opacity: 1 }}
						exit={{ opacity: 0 }}
					/>
