...
import { DurataZbor } from '@/types/customerSupport.type';
import { useDurataZborStore } from '@/lib/store/customerSupport.store';
...
export function DurataZborTable() {
	const data = useDurataZborStore((state) => state.durataZbor);
	const setDurataZbor = useDurataZborStore((state) => state.setDurataZbor);

	const durataZborService = useMemo(() => customerSupportService, []);
	const handleSubmit = async () => {
		try {
			const data = await durataZborService.showDurataZbor();
			setDurataZbor(data);
		} catch (err: any) {
				...
				>
					<MorphingDialogTitle className="text-2xl font-medium">
						{isHovered ? (
							...
							>
								Vezi Cel Mai Lung Zbor
							</TextShimmer>
						) : (
							<span>Vezi Cel Mai Lung Zbor</span>
						)}
					</MorphingDialogTitle>
				</Button>
			</MorphingDialogTrigger>
...
export const columns: ColumnDef<DurataZbor>[] = [
	{
		accessorKey: 'id_orar',
		header: 'ID Orar',
		cell: ({ row }) => <div>{row.getValue('id_orar')}</div>
	},
	{
		accessorKey: 'durata',
		header: 'Durata',
		cell: ({ row }) => <div>{row.getValue('durata')}</div>
	}
];
